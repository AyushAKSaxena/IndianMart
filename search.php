...
$query="select * from warehouseregisteration as wr join warehousebranches as wb on(wr.wid=wb.warehouseid) join farergoodssell as fs on(wb.wbranchid=fs.wbranchid) where Lower(city)='$city' and Lower(fgname) LIKE '%$product%'";
...
{
	if($_REQUEST['gtype']=="perish")
		$gtype = "perishable";
	else
		$gtype = "non-perishable";
	$query .= " and allowed_goods LIKE '%$gtype%' ";
}
